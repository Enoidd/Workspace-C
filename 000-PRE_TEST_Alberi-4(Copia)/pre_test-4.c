...
int numero_nodi_info_distanza_radice_ric(albero t, int i){
    ...
    return numero_nodi_info_distanza_radice_ric(t->l, i+1) + numero_nodi_info_distanza_radice_ric(t->r, i+1);
}
...
int numero_nodi_foglie(albero t){
    ...
    int count = 0;
    if(t->l!=NULL && t->r!=NULL){
        if((t->l->l==NULL && t->l->r==NULL) && (t->r->l==NULL && t->r->r==NULL))
            count++;
    }
